...
k = 32
batch_size = 128
preTraining_epochs = 2 # pre-training epochs --> Auxiliary Task (QA)
...
embedding_index, word_id = utils.read_word_embedding(embedding_path, word_embedding_dim, word_dict_path)
...
QA_train = utils.generate_train_test(QA_train_path, word_id, "QA")
...
with tf.Session() as sess:
    ...
    for epo in range(preTraining_epochs):
        Q_batch, RA_batch, Y_batch = utils.batch_pos_neg(QA_q_train, QA_a_train, QA_y_train, batch_size, 0.5, True)
    ...
    for m in range(fineTune_epochs):
        Q_batch, RA_batch, Y_batch = utils.batch_pos_neg(QR_q_train, QR_r_train, QR_y_train, batch_size, 0.5, True)
        for i in range(len(Y_batch)):
            ...
            step += 1
        # Validation
        Q_dev_batch, R_dev_batch, Y_dev_batch, last_start = utils.batch_triplet_shuffle(QR_q_dev, QR_r_dev, QR_y_dev, batch_size, False)
        prediction = []
