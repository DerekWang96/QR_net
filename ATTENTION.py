...
def BiAttention(Q_gru, R_gru, rnn_dim, batch_size):
    U = tf.get_variable("U", [2 * rnn_dim, 2 * rnn_dim], initializer=tf.truncated_normal_initializer(stddev=0.1))
    G = tf.matmul(tf.matmul(Q_gru, tf.tile(tf.expand_dims(U, 0), [batch_size, 1, 1])), R_gru, adjoint_b=True)
    Q_att = tf.nn.softmax(tf.reduce_max(G, 2))
    R_att = tf.nn.softmax(tf.reduce_max(G, 1))
